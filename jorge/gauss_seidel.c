...
void gauss_seidel_step(double ***u, double ***f, int N) {
    ...
    for (i = 1; i < N - 1; i++) {
        for (j = 1; j < N - 1; j++) {
            for (k = 1; k < N - 1; k++) {
                u[i][j][k] = coef * (u[i-1][j][k] + u[i+1][j][k] + u[i][j-1][k] + u[i][j+1][k] 
                                        + u[i][j][k-1] + u[i][j][k+1] + lambda_2*f[i][j][k]);
            }
        }
    }
}
...
void gauss_seidel(double ***u, double ***u_old, double ***f, int N, int iter_max, double tolerance) {
